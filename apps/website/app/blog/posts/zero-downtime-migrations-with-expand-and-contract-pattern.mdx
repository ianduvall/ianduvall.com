...
- Change API of a Tireswing endpoint
  - Add new version of the endpoint while leaving the existing endpoint around
- Rename the value of a property on a Block
  _ Expand the Block's schema to accept the old and new values at the same time
  _ You can either support both values simultaneously or collapse both values down to one.
...
- Change the API of a Tireswing endpoint
